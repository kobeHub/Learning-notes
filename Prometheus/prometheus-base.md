...
+ 适用于记录纯数字的时序信息
+ 适用于面向服务器等硬件指标的监控，也适用于高动态的面向服务的架构（SOA）的监控
+ 对于微服务架构，prometheus的多维度数据收集以及数据筛选功能也十分强大
+ Prometheus 是为服务可靠性设计的，当服务出现故障时，可以帮助用户快速定位以及诊断问题

### 6.2 不适用场景

+ 不适用于对于监控指标精确性要求高的场景
+ 没有对于日志的支持
+ 不提供长期存储、异常检测、自动水平缩放以及用户管理
+ 不是一个dashboard的解决方案
